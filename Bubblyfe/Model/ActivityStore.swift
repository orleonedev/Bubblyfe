...
class ActivityStore: ObservableObject {
    ...
    var FreeTimedone = Activity(title: "GameSessions", category: "Freetime", details: "See Mike and Mark to talk about gameCon" , reflections: "It was inspiring, really looking forward to the next one", isCompleted: true, completitionDate: Date.now.addingTimeInterval(-(1*24*60*60)), cardColor: Color("Freetime").opacity(0.3), icon: "calendar.circle.fill")
    
    var Extradone = Activity(title: "Learn to do Carbonara", category: "Extra", details: "Search the true recipe" , reflections: "There's no true recipe, everybody can cook how they want and nobody should judge it", isCompleted: true, completitionDate: Date.now.addingTimeInterval(-(1*24*60*60)), cardColor: Color("Extra").opacity(0.3), icon: "star.circle.fill")
    ...
    init() {
        self.activities = [family1active,family2active,study1active,friendsActive,family1done,study1done,FreeTimedone,Extradone]
    }
